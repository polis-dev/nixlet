...
[${GITHUB_ACTOR:-nobody}]: https://github.com/${GITHUB_ACTOR:-nobody}

# ${GITHUB_REPOSITORY:-oops} 

| **key** | **value** |
| ------- | --------------------------- |
| **ref** | \`${GITHUB_REF_NAME:-missing}\` |
| **rev** | \`${GITHUB_SHA:-missing}\` |
| **iid** | \`${INVOCATION_ID:-missing}\` |
...
<details><summary><code>nix.conf</code></summary>

~~~dosini
$(cat "$workdir"/nix.conf)
~~~

**Retained on GitHub for ${GITHUB_RETENTION_DAYS:-0} day(s)**.
